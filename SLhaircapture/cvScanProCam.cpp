...
int slScan(CvCapture* capture , 
		   IplImage**& proj_codes /* list of projection code*/, IplImage**& cam_codes /*initially empty, captured code*/,
		   int n_cols, int n_rows, 
	...
	for(int i=0; i<(n_cols+n_rows+1); i++){
		...
		cam_frame /* this captures image and save to cam_codes */ = cvQueryFrameSafe(capture, sl_params, false); 
...
int runBackgroundCapture(CvCapture* capture, 
	...
	slScan(capture, proj_gray_codes, cam_gray_codes, gray_ncols, gray_nrows, sl_params, sl_calib); /* 쏘고 저장하는 함수 */
	...
	decodeGrayCodes(sl_params->proj_w, sl_params->proj_h,
					cam_gray_codes /* 카메라 저장 패턴 */, 
					/* 이 두 변수 값을 할당하는 함수 */gray_decoded_cols, gray_decoded_rows, sl_calib->background_mask,
...
int runStructuredLight(CvCapture* capture, 
	...
	/* camera의 각각의 픽셀에 대해서 depth 값을 계산하여 저장. masking 되지 않은 모든 픽셀에 대해서. */
	if(sl_params->display)
